...
export const Navbar: React.FC = () => {
  ...
  const navItems = [
    ...
    { id: 'comics', label: 'Blueprint' },
    { id: 'tva-chat', label: 'TVA' },
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-6 h-16 flex items-center justify-between gap-4">
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`px-3 py-1.5 rounded text-xs font-mono font-bold transition-all uppercase tracking-wider ${
                  isActive
        ...
        <div className="flex items-center space-x-2 sm:space-x-3 shrink-0">
          ...
          <div className="hidden xl:flex items-center space-x-2 font-mono text-[10px]">
            <div className="px-2 py-1 bg-[#1a2e1a]/60 border border-[#22c55e]/40 rounded text-[#22c55e]">
              <span>STABLE: {stabilityIndex.toFixed(0)}%</span>
            </div>
            <div className="px-2 py-1 bg-red-950/40 border border-red-500/50 rounded text-red-400">
          ...
          <button
            id="btn-sound-toggle"
            onClick={toggleSound}
            className={`px-2.5 py-1.5 rounded border font-mono text-xs font-bold transition-all ${
              soundEnabled
        ...
        {navItems.map((item) => {
          ...
          return (
            <button
              key={item.id}
              onClick={() => setActiveTab(item.id)}
              className={`whitespace-nowrap px-2.5 py-1 rounded text-xs font-mono font-bold transition-all shrink-0 uppercase ${
                isActive
